...
- `test_end_expr::Expr`: an expression that will be run after each testitem is run.
  By default, the expression is `GC.gc(false); GC.gc(true)`, when `nworkers > 0`, to help with memory pressure,
...
function runtests(
    ...
    test_end_expr::Expr=nworkers>0 ? quote GC.gc(false); GC.gc(true) end : Expr(:block),

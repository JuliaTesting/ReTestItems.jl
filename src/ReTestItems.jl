...
function runtests(
    ...
    if debuglvl > 0
        LoggingExtras.withlevel(LoggingExtras.Debug; verbosity=debuglvl) do
            _runtests(shouldrun_combined, paths′, nworkers, nworker_threads, worker_init_expr, test_end_expr, timeout, retries, memory_threshold, verbose_results, debuglvl, report, logs, failfast, testitem_failfast)
        end
    else
        return _runtests(shouldrun_combined, paths′, nworkers, nworker_threads, worker_init_expr, test_end_expr, timeout, retries, memory_threshold, verbose_results, debuglvl, report, logs, failfast, testitem_failfast)
    end
end
...
function _runtests(shouldrun, paths, nworkers::Int, nworker_threads::String, worker_init_expr::Expr, test_end_expr::Expr, testitem_timeout::Int, retries::Int, memory_threshold::Real, verbose_results::Bool, debug::Int, report::Bool, logs::Symbol, failfast::Bool, testitem_failfast::Bool)
    ...
    with_logger(current_logger()) do
        if is_running_test_runtests_jl(proj_file)
            ...
            return _runtests_in_current_env(shouldrun, paths, proj_file, nworkers, nworker_threads, worker_init_expr, test_end_expr, testitem_timeout, retries, memory_threshold, verbose_results, debug, report, logs, failfast, testitem_failfast)
        else
            ...
            try
                ...
                _runtests_in_current_env(shouldrun, paths, proj_file, nworkers, nworker_threads, worker_init_expr, test_end_expr, testitem_timeout, retries, memory_threshold, verbose_results, debug, report, logs, failfast, testitem_failfast)
            finally
...
function _runtests_in_current_env(
    ...
    failfast::Bool, testitem_failfast::Bool,
)
    start_time = time()
    ...
    try
        ...
        elseif !isempty(testitems.testitems)
            ...
            @sync for (i, w) in enumerate(workers)
                ...
                @spawn begin
                    with_logger(original_logger) do
                        manage_worker($w, $proj_name, $testitems, $ti, $nworker_threads, $worker_init_expr, $test_end_expr, $testitem_timeout, $retries, $memory_threshold, $verbose_results, $debug, $report, $logs, $failfast, $testitem_failfast)
                    end
                end
            end
        end
